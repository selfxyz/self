...
contract IdentityRegistryImplV1 is IdentityRegistryStorageV1, IIdentityRegistryV1 {
    ...
    /// this registry's roots hash against registryHashes[0], and verifies the
    ...
    function updateOfacRootsWithProof(
        ...
        // Verify this registry's roots hash matches registryHashes[0] (IdentityRegistry)
        bytes32 myHash = sha256(abi.encodePacked(roots[0], roots[1], roots[2]));
        if (myHash != registryHashes[0]) revert InvalidRootsHash();
