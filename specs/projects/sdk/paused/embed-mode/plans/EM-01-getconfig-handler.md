...
> Status: **PARKED — code written, unmerged.** See the halt note in
> [embed-mode SPEC](../SPEC.md).

- Workstream: Embed Mode
- Backlog ID: EM-01
- Linear: [SELF-3395](https://linear.app/selfprotocol/issue/SELF-3395)
- Branch: `chore/run-android-locally` in `selfxyz/self-webview-sdk`
- PR: [selfxyz/self-webview-sdk#26](https://github.com/selfxyz/self-webview-sdk/pull/26) (draft)

> **Reading #26 before touching this.** EM-01's deliverable (`HostConfig.kt`,
> `LifecycleBridgeHandler` + tests) is real but **bundled** with unrelated OCR/MRZ,
> NFC, EncryptedSharedPreferences, Makefile and device-script work — 26 files,
> +1875/−141. Do not land it wholesale. Split EM-01 out if the contract is still
> wanted. Android/Kotlin/lint checks pass; **all three iOS jobs are skipped, not
> green**, so Swift-shell parity is unverified.

### Why

- The `getConfig` mode contract is **already settled** in
  `../../../workstreams/webview-in-app/SPEC-MODES.html` and **live for the RN host** (WIA-16, Done). EM-01 does
...
- [ ] Backlog row EM-01 → Done; PR linked; [SELF-3395](https://linear.app/selfprotocol/issue/SELF-3395) updated with the outcome (the issue already exists — do not create a new one).
