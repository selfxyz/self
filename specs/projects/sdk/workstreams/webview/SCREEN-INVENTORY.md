...
#### EU ID Registration Sub-flow (6 screens) — NOT YET MIGRATED
...
Note: Camera/NFC native screens were removed in WV-03. These instruction/viewfinder screens may need to be adapted to the `ProviderLaunchScreen` pattern or kept as pure UI (provider handles actual scan).
...
Open flow decisions:
- whether the EU ID helper screens remain active in a provider-first webview flow
- whether `ConfirmIdentificationScreen` remains a separate registration step
- whether tunnel remains the only proving integration surface or the main proving route becomes primary
