...
- [x] EU ID scope decided (`WV-10` deferred to mobile app 3.1; not part of initial webview release)
...
These sit inside the legacy registration journey, but the active webview scope no longer assumes Self-owned camera or NFC capture. Per WV-10, EU ID is not part of provider-backed registration and is not part of the initial webview release. The six screens are deferred to mobile app 3.1 unless a separate Self-owned flow is approved in a future spec.
...
| `WV-10`       | EU ID defer record for post-release follow-up                           | WV-09        |
...
- `WV-10` is intentionally not on the initial webview release path. EU ID is not part of provider-backed registration, and the six screens are deferred to mobile app 3.1 unless a separate Self-owned flow is approved later.
...
- `Carry EU ID as a mobile app 3.1 follow-up instead of an active webview migration item`
...
3. `WV-07` and `WV-08` implementation tickets if still not created
4. `WV-11` disclose spine
5. `WV-12` proof overlays and post-proof surfaces
6. `WV-13` home and document surfaces
7. `WV-15` settings support routes
8. `WV-14` recovery and backup
...
1. Resolved: EU ID is not part of the provider-backed registration flow and is out of scope for the initial webview release. Track it as mobile app 3.1 follow-up work.
