...
- **B1 relayer completion (SELF-3583) — DONE (committed).** Host owns the single relayer socket; `WebViewHostScreen.onSuccess/onFailure` emit `proof_verified`/`proof_generation_failed` via `getSelfAppState().handleProofResult`, guarded by a `sessionId` correlation check and socket teardown on unmount. Also fixed `LifecycleHandler.setResult` reading only flat `errorCode/errorMessage` (WebView sends nested `error:{code,message}`, so failures previously fell through to `onCancelled`). On-device TODO: verify the emit flushes before the unmount disconnect; confirm end-to-end `sessionId` correlation.
  ...
  - **B1 follow-up — DEFERRED → SELF-3638 (single-owner socket / emit on proving-terminal).** Relay completion is still coupled to the result-screen Continue button (`lifecycle.setResult` from `DiscloseResultScreen`/`VerificationResultScreen.handleContinue`), so closing the app after proving-terminal but before Continue never notifies the website (Codex #2221). Root cause is the split-brain socket. **Recommended design (approach C):** `@selfxyz/rn-sdk` owns the relayer socket — it runs in the RN runtime (same backgrounding resilience as today's host socket, unlike a WebView-owned socket) and emits at proving-terminal via `LifecycleHandler`, with the WebView forwarding `SdkEvents.VERIFICATION_COMPLETE` at terminal (not the button). Centralizes the emit in the SDK so partner wallets get it for free; embed/KYC unaffected (opt-in). Alternatives considered: A (WebView owns socket — weakest resilience) and B (host keeps socket + bridge signal — duplicated per host). Idempotency: the `resultSent` flag in the result screens is scaffolded but never set. Full plan + acceptance criteria in SELF-3638 (closes SELF-3583 AC-2).
