...
The KMP SDK (`packages/kmp-sdk/`) already contains **complete, tested bridge handler implementations** for both NFC and Camera/MRZ on both platforms. These were stripped from handler registration during KMP Revival (SELF-2488) but the code was explicitly retained. The TypeScript bridge layer also already defines the domains, methods, adapters, and types.
...
Rationale:
- **Android:** The KMP handler is a self-contained implementation using standard Android APIs (`NfcAdapter`, CameraX, ML Kit). No consumer-specific configuration needed beyond NFC hardware availability. Embedding keeps integration simple — one dependency, it works.
