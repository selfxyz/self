...
The interactive widget in [SPEC.html](./SPEC.html#open-questions) and each plan page is the *editing surface* — this file is the *durable source of truth*. If the two ever disagree, update both.
...
**Decision:** Navigate to `/embed/error` *before* `setResult` + `dismiss`.
...
**Action item:** This decision creates a *new* Euclid screen requirement. See [NAV-03 plan → Action Items](./plans/NAV-03-boot-decision.html#action-items) for the screen spec + team-communication checklist.
