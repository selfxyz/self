# SC-03: Extract Reusable SelfClient Adapter Assembly
...
You are eliminating duplicated adapter assembly logic across two consumers of
`createSelfClient()`.

Today, both `app/src/providers/selfClientProvider.tsx` (519 lines) and
`packages/webview-app/src/providers/SelfClientProvider.tsx` (108 lines) manually
construct adapter objects. The app provider is the worst offender — it inlines
crypto hashing, WebSocket wrapping, navigation plumbing, and analytics wiring
that are not app-specific. The webview-app provider is cleaner because it uses
bridge adapter factories from `@selfxyz/webview-bridge/adapters`, but it still
wires them into a bespoke `SelfClientAdapters` type rather than the SDK's own
`Adapters` interface.

The result:

1. Every new adapter method (like `generateKey`/`getPublicKey` in SC-02) must be
   patched in every consumer — the app CI broke because its inline crypto
   adapter was missing the new methods.
...
4. Neither consumer validates that the assembled `Adapters` object is complete
   before passing it to `createSelfClient()` — type errors only surface at build
   time, not at assembly time.

### What exists today

| Consumer | File | LOC | Duplicated adapter assembly |
|----------|------|-----|----------------------------|
| RN app | `app/src/providers/selfClientProvider.tsx` | 519 | crypto (hash+stubs), network (fetch+ws), navigation, analytics relay |
| WebView app | `packages/webview-app/src/providers/SelfClientProvider.tsx` | 108 | Bridge adapters wired into bespoke type, lifecycle.ready() call |
...
| File | Why |
|------|-----|
| `packages/webview-app/src/providers/SelfClientProvider.tsx` | Already uses bridge adapter factories — different assembly path. Future work may align its `SelfClientAdapters` type with the SDK's `Adapters`, but that's a separate concern. |
...
- **Don't touch the webview-app provider yet.** It has its own adapter type
  (`SelfClientAdapters`) that includes lifecycle and biometrics — adapters the
  SDK's `Adapters` interface doesn't define. Aligning those types is a separate
  follow-up.
