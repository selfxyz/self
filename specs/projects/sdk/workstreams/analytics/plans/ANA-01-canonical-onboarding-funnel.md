...
| `OnboardingEvents.PROOF_STARTED`          | `Onboarding: Proof Generation Started`   | `packages/mobile-sdk-alpha/src/proving/provingMachine.ts` — when state enters `proving` and `circuitType !== 'disclose'`. The disclose gate prevents disclosure flows from bootstrapping a fake onboarding attempt via `ensureAttempt`. See ANA-11. | —                                                                                       |
