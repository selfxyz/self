...
import { getDscTreeInclusionProof, getLeafCscaTree, getLeafDscTree } from '../../trees/index.js';
import { formatInput } from './format.js';

export function generatePassportRegisterInputs(
  secret: string,
  passportData: PassportData,
  serializedDscTree: string,
  useTestPadding: boolean = false,
) {
  ...
  if (
    eContent.length >
    MAX_PADDED_ECONTENT_LEN[signatureAlgorithmFullName as keyof typeof MAX_PADDED_ECONTENT_LEN]!
  ) {
    console.error(
  ...
  const signedAttrLenMap = useTestPadding
    ? MAX_PADDED_SIGNED_ATTR_LEN_FOR_TESTS
    : MAX_PADDED_SIGNED_ATTR_LEN;
  ...
  const [signedAttrPadded, signedAttrPaddedLen] = pad(passportMetadata.signedAttrHashFunction)(
    signedAttr,
    signedAttrLenMap[passportMetadata.eContentHashFunction],
  );
