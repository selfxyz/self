...
export const formatRevealedDataPacked = (attestationId: AttestationId, publicSignals: string[]): GenericDiscloseOutput => {
  ...
  const name = revealedDataPackedString.subarray(revealedDataIndices[attestationId].nameStart, revealedDataIndices[attestationId].nameEnd + 1).toString("utf-8").replace("<<", " ").replace(/<*/g, "");
  const idNumber = revealedDataPackedString.subarray(revealedDataIndices[attestationId].idNumberStart, revealedDataIndices[attestationId].idNumberEnd + 1).toString("utf-8");
  const nationality = revealedDataPackedString.subarray(revealedDataIndices[attestationId].nationalityStart, revealedDataIndices[attestationId].nationalityEnd + 1).toString("utf-8");
