...
import {generateCommitmentAadhaar, prepareTestData, splitTestData} from "../../src/utils/aadhaar/aadhaar";
...
describe('utils - Produce desired commitment',function() {
  it('should generate the data commitment and final commitment for aadhaar registration ',async function (){
    ...
    const { ReferenceId } = splitTestData(qrDataPadded, delimiterIndices);
    const RefId = ProcessReferenceId(ReferenceId);

    const actualCommitment = await generateCommitmentAadhaar(
      secret,
      attestationId,
      qrDataPadded
    );
