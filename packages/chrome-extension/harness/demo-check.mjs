// CE-05/06 e2e wiring check: spawns the demo RP (backend + vite), imports a
// mock account into the extension, then drives the page:
//   - shim detects the extension through the content script (ping/pong)
//   - "Verify with the Self extension" opens the approval popup for the
//     page's session (background routing)
//   - closing the popup routes USER_CANCELLED back to the page via the shim
//   - fail-closed overlap: a second request while one is pending gets BUSY
// The happy path (real TEE proof + backend verification) needs a registered
// account + tunnel: manual QA.
//
// Usage: node harness/demo-check.mjs [--headed]
...
const ACCOUNT = {
  ...
  documentCatalog: { documents: [{ id: DOC_ID, documentType: 'mock_passport', documentCategory: 'passport', data: 'P', mock: true, isRegistered: true }], selectedDocumentId: DOC_ID },
...
function importAccount(qr) {
  return new Promise((resolvePromise, reject) => {
    const socket = io(`${qr.relay}/websocket`, { path: '/', transports: ['websocket'], forceNew: true, query: { sessionId: qr.transferSessionId, clientType: 'web' } });
    ...
    socket.on('mobile_status', data => {
      if (data?.status === 'mobile_connected') {
        ...
        socket.emit('self_app', { sessionId: qr.transferSessionId, transferType: 'self-account-transfer', senderPublicKey: ecdh.getPublicKey('hex', 'uncompressed'), envelope: encryptEnvelope(shared, Buffer.from(JSON.stringify(ACCOUNT))) });
      }
...
function spawnServer(command, args, cwd, readyMatch) {
  return new Promise((resolvePromise, reject) => {
    ...
    const timer = setTimeout(() => reject(new Error(`${command} ${args.join(' ')} never became ready`)), 60_000);
    const onData = chunk => {
...
let browser;
try {
  servers.push(await spawnServer('node', ['backend/server.mjs'], demoRoot, /listening on/));
  ...
  // 1. Import the mock account.
  const setup = await browser.newPage();
  await setup.goto(`chrome-extension://${EXTENSION_ID}/link.html`, { waitUntil: 'load' });
  await setup.waitForSelector('#qr[data-qr-content]', { timeout: 15_000 });
  const qr = JSON.parse(await setup.$eval('#qr', node => node.dataset.qrContent));
  const importDone = importAccount(qr);
  await setup.waitForSelector('#step-password:not(.hidden)', { timeout: 60_000 });
  ...
  // 2. Demo page detects the extension via the shim.
  const page = await browser.newPage();
  page.on('console', msg => {
    if (msg.type() === 'error' || msg.type() === 'warn') console.log(`[demo:${msg.type()}] ${msg.text().slice(0, 200)}`);
  });
  page.on('pageerror', err => console.log(`[demo:pageerror] ${err.message.slice(0, 300)}`));
  await page.goto(DEMO_URL, { waitUntil: 'networkidle2' });
  try {
    await page.waitForFunction(() => document.body.innerText.includes('Verify with the Self extension'), { timeout: 20_000 });
  } catch (err) {
  ...
  // 3. Clicking opens the approval popup for the page's session.
  ...
  if (!popupUrl.includes('ext_mode=embed') || !popupUrl.includes('verificationId=')) {
    throw new Error('popup URL missing embed params');
  }
  const sessionId = await page.evaluate(() => document.body.innerText.match(/session ([0-9a-f-]{36})/)?.[1]);
  if (sessionId && !popupUrl.includes(sessionId)) throw new Error('popup session differs from page session');

  // 4. Overlap fails closed while the first request is pending.
  const busy = await page.evaluate(async () => {
    const mod = { requestVerification: null };
    return await new Promise(resolveInner => {
      function onMessage(event) {
        const data = event.data;
        if (data?.type === 'self:ext:result' && data.sessionId === 'overlap-probe-session') {
          window.removeEventListener('message', onMessage);
          resolveInner(data.result?.error?.code ?? 'NO_ERROR');
        }
      }
      window.addEventListener('message', onMessage);
      window.postMessage({ type: 'self:ext:request', selfApp: { sessionId: 'overlap-probe-session', scope: 's', userId: 'u' } }, window.origin);
      setTimeout(() => resolveInner('TIMEOUT'), 5000);
    });
  });
  if (busy !== 'BUSY') throw new Error(`overlap probe expected BUSY, got ${busy}`);
  console.log('[harness] overlapping request rejected with BUSY');

  // 5. Closing the popup routes USER_CANCELLED back to the page.
  const popupPage = await popupTarget.page();
  await popupPage.close();
  await page.waitForFunction(() => document.body.innerText.includes('USER_CANCELLED'), { timeout: 20_000 });
