...
async function readFailures(): Promise<FailureRecord> {
  ...
  return (
    (record[FAILED_ATTEMPTS_KEY] as FailureRecord | undefined) ?? {
      count: 0,
      blockedUntil: 0,
    }
  );
...
async function recordFailure(): Promise<void> {
  ...
  const delay =
    over > 0
      ? Math.min(THROTTLE_MAX_MS, THROTTLE_STEP_MS * 2 ** (over - 1))
      : 0;
  await chrome.storage.session.set({
    [FAILED_ATTEMPTS_KEY]: {
      count,
      blockedUntil: delay > 0 ? Date.now() + delay : 0,
    },
  });
}
...
export async function initialize(password: string): Promise<void> {
  if (await isInitialized())
    throw new Error('A vault already exists in this browser; reset it first');
  if (password.length < MIN_PASSWORD_LENGTH) {
    throw new Error(
  ...
  const canary = await encryptEnvelope(
    key,
  ...
  return {
    async get(key: string): Promise<string | null> {
      ...
      return new TextDecoder().decode(
        await decryptEnvelope(cryptoKey, envelope),
      );
    },
