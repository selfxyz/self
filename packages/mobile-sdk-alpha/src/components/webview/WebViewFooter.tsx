...
import { black, slate50, slate400 } from '../../constants/colors';
...
export const WebViewFooter: React.FC<WebViewFooterProps> = ({
  ...
  const renderIconButton = (key: string, icon: React.ReactNode, onPress: () => void, disabled?: boolean) => (
    <Button
      key={key}
      size="$4"
      unstyled
  ...
  return (
    <YStack gap={12} paddingVertical={12} width="100%">
      <XStack justifyContent="space-between" alignItems="center" width="100%">
        ...
        {renderIconButton('reload', <RotateCcw size={iconSize} color={black} />, onReload)}
        {renderIconButton(
          'forward',
          <ArrowRight size={iconSize} color={canGoForward ? black : slate400} />,
          onGoForward,
          !canGoForward,
        )}
      </XStack>
