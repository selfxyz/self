...
import { Additional, Description, SecondaryButton, Text, Title, View, XStack, YStack } from 'src/components';
...
export const DocumentCameraScreen = ({ onBack }: Props) => {
  ...
  const handleMRZDetected = useCallback((mrzData: MRZInfo) => {
  ...
  const handleScannerError = useCallback((error: string) => {
    selfClient.emit(
      SdkEvents.ERROR,
      new Error(`MRZ scanner error: ${error}`)
    );
  }, [selfClient]);
    ...
    <ExpandableBottomLayout.Layout backgroundColor={white}>
      <ExpandableBottomLayout.TopSection backgroundColor={black}>
        <MRZScannerView
          onMRZDetected={handleMRZDetected}
