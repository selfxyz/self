...
import { fetchAllTreesAndCircuits } from '../../stores';
import type { SelfClient } from '../../types/public';
import type { ProvingDependencies } from './dependencyFactory';
import { checkActorInitialized, createProofContext } from './helpers';

export const parseIDDocument = async (selfClient: SelfClient, deps: ProvingDependencies) => {
...
export const startFetchingData = async (selfClient: SelfClient, deps: ProvingDependencies) => {
  const actor = deps.getActor();
  checkActorInitialized(actor);
  const get = deps.get;
...
export const validatingDocument = async (
  selfClient: SelfClient,
  deps: ProvingDependencies,
  getCommitmentTree: (selfClient: SelfClient, documentCategory: DocumentCategory) => unknown,
) => {
  const actor = deps.getActor();
  checkActorInitialized(actor);
  const get = deps.get;
  ...
  try {
    ...
    if (circuitType === 'disclose') {
      const isRegisteredWithLocalCSCA = await isUserRegistered(
        passportData,
        secret as string,
        (documentCategory: DocumentCategory) => getCommitmentTree(selfClient, documentCategory) as string,
      );
    ...
    /// registration
      ...
      const { isRegistered, csca } = await isUserRegisteredWithAlternativeCSCA(passportData, secret as string, {
        getCommitmentTree: (docCategory: DocumentCategory) => getCommitmentTree(selfClient, docCategory) as string,
