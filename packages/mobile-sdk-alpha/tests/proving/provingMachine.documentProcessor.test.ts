...
describe('validatingDocument', () => {
  ...
  it('throws error when KYC document type is used during registration', async () => {
    const passportData = buildPassportFixture();
    // KYC document type is not yet fully implemented - using type assertion to test error handling
    const kycDocument = {
      ...passportData,
    ...
    loadSelectedDocumentMock.mockResolvedValue({ data: kycDocument } as any);
    ...
    try {
      ...
      useProvingStore.setState({ passportData: kycDocument as any, secret, circuitType: 'register' });

      await useProvingStore.getState().validatingDocument(selfClient);

      expect(actorMock.send).toHaveBeenCalledWith({ type: 'VALIDATION_ERROR' });
      expect(selfClient.trackEvent).toHaveBeenCalledWith(
        ProofEvents.VALIDATION_FAILED,
        expect.objectContaining({
          message: 'KYC is not supported yet',
        }),
      );
    } finally {
      fetchSpy.mockRestore();
