...
class MessageRouterTest {
    ...
    @Test
    fun routes_to_registered_handler() = runTest {
    ...
    @Test
    fun returns_error_for_unknown_domain() = runTest {
        val responses = mutableListOf<String>()
    ...
    @Test
    fun returns_error_when_handler_throws() = runTest {
        val responses = mutableListOf<String>()
        val testScope = TestScope(UnconfinedTestDispatcher(testScheduler))
        val router = MessageRouter(
            sendToWebView = { responses.add(it) },
        ...
        router.register(object : BridgeHandler {
            ...
            override suspend fun handle(method: String, params: Map<String, JsonElement>): JsonElement? {
                throw BridgeHandlerException("KEY_NOT_FOUND", "No such key")
            }
        })
