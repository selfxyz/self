...
/**
 * npm postinstall: fetch the prebuilt iOS MRZ binary into ios/Frameworks/.
 *
 * The MRZ Vision engine ships as a prebuilt binary xcframework (RN-08 policy): it is large,
 * private, and must not live in the npm tarball. CocoaPods reads whatever this script places in
 * ios/Frameworks/ via the podspec's conditional `vendored_frameworks`. When the binary is
 * absent the Swift module compiles in its `#if canImport(SelfSdkOcr)` stub branch, so
 ...
 *                              (default: NATIVE_ARTIFACT_VERSION below). The published assets are
 *                              named SelfSdkOcr-<version>.xcframework.zip etc.
...
// The MRZ Vision engine xcframework vendored by selfxyz-rn-mrz-scanner.podspec. SelfSdkOcr is a
// static lib with no SelfSdkProviders/SelfSdk symbols — SelfSdkProviders is source-distributed and
// not needed here, so vendor SelfSdkOcr alone.
const ARTIFACTS = ['SelfSdkOcr'];

const log = (msg) => console.log(`[@selfxyz/rn-mrz-scanner] ${msg}`);
const warn = (msg) => console.warn(`[@selfxyz/rn-mrz-scanner] ${msg}`);
...
async function download(name, version, tag, release) {
  ...
  const apiHeaders = {
    ...
    'user-agent': 'selfxyz-rn-mrz-scanner-postinstall',
  };
...
// Locate an unzip tool. `ditto` (macOS) preserves the .xcframework structure best;
  ...
  if (!usingOverride && !token) {
    warn(
      'no GitHub token found (set SELF_SDK_GITHUB_TOKEN, GITHUB_TOKEN, or GH_TOKEN with read ' +
        `access to ${DIST_REPO}) — skipping iOS MRZ binary download. iOS will compile the ` +
        'unavailable stub and MRZ will report not available. (Android-only setups can ignore this.)'
    );
    return;
  }

  // The iOS xcframework is versioned with the native self-sdk-native release (self-sdk-dist
  // rn-v* tag), NOT this shim's npm version. Keep in lockstep with the AAR pin in
  // android/build.gradle; override via SELF_SDK_DIST_VERSION.
      ...
      if (err.status === 404) {
        warn(
          `release ${tag} not found on ${DIST_REPO} (or the token lacks access) — skipping iOS ` +
            'MRZ binary download; iOS will compile the unavailable stub.'
        );
        return;
      }
      if (err.status === 401 || err.status === 403) {
        warn(
          `not authorized to read ${DIST_REPO} (HTTP ${err.status}) — skipping iOS MRZ binary ` +
...
main().catch((err) => {
  ...
  console.error(`[@selfxyz/rn-mrz-scanner] postinstall failed: ${err.message}`);
  process.exit(1);
});
