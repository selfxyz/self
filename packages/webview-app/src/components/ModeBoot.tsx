 ...
 *   1. Embed-mode guard: if mode === 'embed' but the host did not supply a
 *      valid verificationRequest, emit setResult({ success: false,
 *      errorCode: 'INVALID_REQUEST' }) then dismiss.
 *   2. Initial routing: embed boots into /tunnel/tour/1; self-app stays at /.
 *   3. Cross-mode navigation guard: embed users that find themselves on
 *      non-/tunnel routes get pulled back into the embed flow; self-app
 *      users on /tunnel/* routes get pulled back home.
 *
...
export const ModeBoot: React.FC = () => {
  ...
  const embedGuardFiredRef = useRef(false);

  useEffect(() => {
    if (!isReady) return;
    if (mode !== 'embed') return;
    if (embedGuardFiredRef.current) return;
    if (hasValidVerificationRequest(verificationRequest)) return;

    embedGuardFiredRef.current = true;
    void (async () => {
      try {
