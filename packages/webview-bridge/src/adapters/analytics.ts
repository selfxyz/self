...
export interface BridgeAnalyticsAdapter {
  ...
  logNFCEvent(level: string, message: string, context: Record<string, unknown>, details?: Record<string, unknown>): void;
}
...
export function bridgeAnalyticsAdapter(bridge: WebViewBridge): BridgeAnalyticsAdapter {
  return {
    ...
    logNFCEvent(level: string, message: string, context: Record<string, unknown>, details?: Record<string, unknown>): void {
      bridge.fire('analytics', 'logNfcEvent', { level, message, context, details });
    },
  };
}
