...
class MessageRouterTest {
    ...
    @Test
    fun `unsupported version sends error response`() =
        runTest {
    ...
    @Test
    fun `unknown domain sends error response`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
    ...
    @Test
    fun `successful handler call sends success response`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
    ...
    @Test
    fun `handler exception sends error response with code`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
            router.register(
    ...
    @Test
    fun `generic exception sends INTERNAL_ERROR response`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
            router.register(
    ...
    @Test
    fun `malformed json is silently dropped`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
    ...
    @Test
    fun `pushEvent sends formatted event to webview`() =
        runTest {
            val sent = mutableListOf<String>()
            val router = MessageRouter(sendToWebView = { sent.add(it) }, scope = this)
