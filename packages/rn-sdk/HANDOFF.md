...
## RN Test Harness Status (`packages/rn-test-app`)

- A minimal React Native host app now exists at `packages/rn-test-app/` for real-device integration validation of `@selfxyz/rn-sdk`.
...
- CI coverage added via `.github/workflows/rn-test-app-ci.yml` to typecheck both `@selfxyz/rn-sdk` and `@selfxyz/rn-test-app` on relevant path changes.
