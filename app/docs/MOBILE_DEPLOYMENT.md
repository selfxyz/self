...
### Build Android for Manual Play Console Upload

Use this when Play Store upload is blocked but you do not want to build locally or manage local environment variables.
...
3. Select:
   - Platform: `android`
   - Test mode: checked
   - Deployment track: the track you plan to upload in Play Console
   - Version bump: usually `build` unless you need a semantic version bump
4. Open the completed run summary and click the signed AAB artifact link.
5. Download the artifact ZIP, extract `app-release.aab`, and upload that file manually in Play Console.

The artifact link is generated by GitHub Actions and is retained for 7 days.
