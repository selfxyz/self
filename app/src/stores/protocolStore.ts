...
import { API_URL, API_URL_STAGING, CSCA_TREE_URL, CSCA_TREE_URL_STAGING, DSC_TREE_URL, DSC_TREE_URL_STAGING, IDENTITY_TREE_URL, IDENTITY_TREE_URL_STAGING } from '../../../common/src/constants/constants';
...
export const useProtocolStore = create<ProtocolState>((set, get) => ({
    passport: {
        ...
        fetch_deployed_circuits: async (environment: 'prod' | 'stg') => {
            const response = await fetch(`${environment === 'prod' ? API_URL : API_URL_STAGING}/deployed-circuits`);
            const data = await response.json();
            set({ passport: { ...get().passport, deployed_circuits: data.data } });
        },
        fetch_circuits_dns_mapping: async (environment: 'prod' | 'stg') => {
            const response = await fetch(`${environment === 'prod' ? API_URL : API_URL_STAGING}/circuit-dns-mapping`);
            const data = await response.json();
            set({ passport: { ...get().passport, circuits_dns_mapping: data.data } });
        },
        fetch_csca_tree: async (environment: 'prod' | 'stg') => {
            const response = await fetch(`${environment === 'prod' ? CSCA_TREE_URL : CSCA_TREE_URL_STAGING}`);
            const data = await response.json();
            set({ passport: { ...get().passport, csca_tree: data.data } });
        },
        fetch_dsc_tree: async (environment: 'prod' | 'stg') => {
            const response = await fetch(`${environment === 'prod' ? DSC_TREE_URL : DSC_TREE_URL_STAGING}`);
            const data = await response.json();
            set({ passport: { ...get().passport, dsc_tree: data.data } });
        },
        fetch_identity_tree: async (environment: 'prod' | 'stg') => {
            const response = await fetch(`${environment === 'prod' ? IDENTITY_TREE_URL : IDENTITY_TREE_URL_STAGING}`);
            const data = await response.json();
